...
def loss_probability(tile: int, known_pile: list[int], tileDeck: list[int], discards: list[int],
                     gammaLUT: list[float] = ()) -> float:
...
def findOptimalDiscard_enhanced(inner_hand: list[int], outer_hand: list[int], known_pile: list[int],
                                discards: list[int],
    ...
    for t in tilesToConsider:
        ...
        match goal:
            ...
            case _:
                potentialScore: int = 16

        # print(handValueAfterdiscard * potentialScore)
        tile_params = np.array([hValue * potentialScore, 0])
...
if __name__ == '__main__':
    ...
    print(findOptimalDiscard_enhanced(i_hand, o_hand, domain, td, td))
